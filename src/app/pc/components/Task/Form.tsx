...
const TaskForm: React.FC<FormValues> = ({
  ...
  useEffect(() => {
    if (
      ...
      initialValues.salary.minMoney !== "" &&
      initialValues.salary.maxMoney !== ""
    ) {
      console.log("initialValues", initialValues);
      const newValue = {
        ...initialValues,
        salary: {
          minMoney: initialValues.salary.minMoney,
          maxMoney: initialValues.salary.maxMoney,
        },
      };
  ...
  const addFilterPassCompany = () => {
    ...
    passArr = passArr.filter((item) => item.trim() !== "");
  ...
  const handleSelectChange = (newValue: string[], option: any, key: string) => {
      ...
      <Form.Item label="投递城市" rules={[{ required: false }]}>
        <Select
          ...
          filterOption={(input, option: any) =>
            (option?.keyword ?? "").toLowerCase().includes(input.toLowerCase())
          }
        />
        ...
        <div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="col-span-3 flex items-center gap-2">
              <p>
                (建议填写,不填写的话会乱投;
                譬如找技术岗位,则填写技术的关键字Java,或者Js,如果是销售,则填写销售...)
              </p>
            </div>
          </div>
          <div className="flex w-full flex-row flex-wrap gap-2 py-5">
            ...
            {(formValue.positionKeywords || []).map((position: KeyWordsMap) => (
              <Tag
                key={position.id}
                closable
                onClose={() => {
      ...
      <Form.Item label="过滤职位详情关键字">
        <div>
          ...
          <div className="flex w-full flex-row flex-wrap gap-2 py-5">
            ...
            {(formValue.filteredKeywords || []).map((compony: KeyWordsMap) => (
              <Tag
                key={compony.id}
                closable
                onClose={() => {
